...
Tools for parsing the `GCS Plan to get to CDR.md` document and generating multiple visual summaries of the Science / Systems Engineering / Software workload.
...
- Packages: `matplotlib`, `matplotlib-venn`, `networkx`, `numpy` (install via `pip` in the environment if needed).
...
```
Each script reads `GCS Plan to get to CDR.md` from the repo root and writes PNG files alongside the source.
